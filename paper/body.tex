...
justifications, \texttt{vulnerable\_code\_not\_present}, is a claim about the
...
Symbol table absence is a natural justification for
\texttt{vulnerable\_code\_not\_present}. It is unsound for internal functions in
a way that is invisible from outside: the check returns absent on every build
...
\texttt{vulnerable\_code\_not\_present} asserts a fact about the artifact, and is
...
(\texttt{vulnerable\_code\_not\_present}) fires when the component is present but
...
al.~\cite{cofano} report that low SBOM precision can lose on the order of 80\% of
vulnerabilities relative to ground truth in the Python ecosystem; Benedetti et
al.~\cite{benedetti} estimate that roughly 20\% of vulnerabilities may be
identifiable from generated SBOMs at current generator precision; and Dann et
al.~\cite{dann} report a true positive rate near 34\% on a Java oriented scanner
evaluation. We report these figures as characterised in that survey rather than
...
The dominant reason codes are \texttt{purl\_match\_insufficient} (6,591),
...
CVE-2026-14456 & \texttt{port\_default\_packet\_handler} & \texttt{libssl.so.3} & 0 & 691 DTLS references; 13 exported DTLS symbols; 43 DTLS strings \\
...
evidence. One symbol in four. The five surviving \texttt{affected} verdicts in
the post guard corpus run on this path under D04.

Across the corpus the 35 unsound clearances distribute as 22 instances of
CVE-2026-14456, 9 of CVE-2026-42767 and 4 of CVE-2026-45445.
...
immediate practical consequence than anything above. It concerns a widely used
open source scanner reading SBOMs produced by a widely used open source SBOM
generator. Both findings below were reported upstream with reproductions before
publication.
...
CycloneDX SBOM of the same tar, then scanning that SBOM with the same scanner
...
packages that install. The generator records the source package twice, as a
namespaced property and as a PURL qualifier. The scanner's SBOM ingest path reads
...
between the two counts is 272, which is 278 minus 6. Copying the value the
generator already wrote into the property the scanner reads produces 415, which
...
control that carries weight is a second scanner, which matches its own direct
...
The same generator asked for SPDX rather than CycloneDX, on the same tar,
produces a document carrying 151 packages with valid PURLs. The scanner retains
...
SPDX path the scanner logs that it detected no operating system, warns that the
family is unsupported, and never runs OS package detection at all. On the
CycloneDX path, from the same generator run on the same tar, it detects the
...
This reproduces across generator versions sixteen months apart. A second scanner
